...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // gitHub相关API
  getEncrypts: () => ipcRenderer.invoke('get-encrypts'),
  saveEncrypts: (data, isEncypt) => ipcRenderer.invoke('save-encrypts', data, isEncypt),
  getRepoTree: (data) => ipcRenderer.invoke('get-repo-tree', data),
  downLoadFileFromGithub: (data, path) => ipcRenderer.invoke('down-load-file-from-github', data, path),
  selectFolder: () => ipcRenderer.invoke('select-folder'),
  compressFolder: (folderPath) => ipcRenderer.invoke('compress-folder', folderPath),
  upLoadFileToGithub: (data, zipPath) => ipcRenderer.invoke('up-load-file-to-github', data, zipPath),
  ...
  // 主进程与渲染进程通信
  onMessage: (callback) => {ipcRenderer.on('message-from-main', (_, message) => callback(message));}
});
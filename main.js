...
const GitOperator = require('./app/main/git-operator');
...
class ScriptManagerApp {
  ...
  setupIPC() {
	...
	// gitHub相关IPC
	ipcMain.handle('get-encrypts', async () => {
      try {
        return await this.gitOperator.getEncrypts();
      } catch (error) {
	...
	ipcMain.handle('save-encrypts', async (event, data, isEncypt) => {
      try {
        return await this.gitOperator.saveEncrypts(data, isEncypt);
      } catch (error) {
    ...
    ipcMain.handle('get-repo-tree', async (event, data) => {
      try {
        return await this.gitOperator.getRepoTree(data);
      } catch (error) {
        console.error('获取仓库目录树失败:', error);
        return { success: false, error: error.message };
      }
    });
	
	ipcMain.handle('down-load-file-from-github', async (event, data, path) => {
      try {
        return await this.gitOperator.downLoadFileFromGithub(data, path);
      } catch (error) {
        console.error('下载仓库文件失败:', error);
        return { success: false, error: error.message };
      }
    });
	...
	ipcMain.handle('select-folder', async () => {
	  try {
	    return await this.gitOperator.selectFolder();
	  } catch (error) {
		console.error('打开文件夹失败:', error);
        return { success: false, error: error.message };
      }
	...
	ipcMain.handle('compress-folder', async (event, folderPath) => {
	  try {
	    return await this.gitOperator.compressFolder(folderPath);
	  } catch (error) {
	...
	ipcMain.handle('up-load-file-to-github', async (event, data, zipPath) => {
	  try {
	    return await this.gitOperator.upLoadFileToGithub(data, zipPath);
	  } catch (error) {
	...
	// 监听任务完成事件并转发给渲染进程
    ipcMain.on('task-completed-event', (event, data) => {
      if (this.mainWindow) {
        this.mainWindow.webContents.send('message-from-main', {type: 'update-task-status', data });
      }
    });
  }

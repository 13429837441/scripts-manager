...
class ScriptExecutor {
  ...
  async launchScript(scriptId, scriptData) {
    try {
      ...
      // 根据脚本类型确定启动命令
      const command = this.getScriptCommand(scriptData.type, scriptData.path, scriptData.params);
      ...
      if (process.platform === 'win32') {
        ...
        const absoluteCommand = this.getScriptCommand(scriptData.type, absoluteScriptPath, scriptData.params);
  ...
  // 获取脚本启动命令
  getScriptCommand(type, scriptPath, scriptParam) {
	const params = JSON.parse(scriptParam);
    const commands = {
      python: {
        // 根据文件扩展名选择合适的Python执行器
        cmd: this.getPythonExecutor(scriptPath),
        args: ['-u', scriptPath].concat(params)
      },
      javascript: {
        cmd: 'node',
        args: [scriptPath].concat(params)
      },
      typescript: {
        cmd: 'ts-node',
        args: [scriptPath].concat(params)
      ...
      batch: {
        cmd: 'cmd',
        args: ['/c', scriptPath].concat(params)
      },
      powershell: {
        cmd: 'powershell',
        args: ['-ExecutionPolicy', 'Bypass', '-File', scriptPath].concat(params)
      },
      bash: {
        cmd: 'bash',
        args: [scriptPath].concat(params)

...
class ScriptManager {
  ...
  initializeElements() {
    ...
    this.elements.loadScriptBtn = document.getElementById('upload-btn');
  ...
  setupEventListeners() {
	...
	// 访问脚本库按钮
    this.elements.loadScriptBtn?.addEventListener('click', () => {
	...
	// 监听主进程的任务更新通知
	this.setupMainUpdateTaskMessage();
  }

  setupMainUpdateTaskMessage() {
	window.electronAPI.onMessage((message) => {
	  ...
	  if (message.type === 'update-task-status') {
		...
		if (modalOverlay.style.display !== 'none') {
		  ...
		  window.taskManager.refreshTaskList();
		}
	  } 
	});
  }
      ...
      timerInfoHtml = `
        <div class="card-timer-info">
          <span class="timer-icon">
			<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"></path><path d="M22 12A10 10 0 0 0 12 2v10z"></path></svg>
		  </span>
  ...
  getAddScriptModalContent() {
    return `
      <form id="add-script-form">
        ...
        <div class="form-group">
          <label class="form-label">脚本参数</label>
          <textarea class="form-textarea" id="script-params" placeholder='格式：["param1", "param2", ...]'></textarea>
        </div>
  ...
  getEditScriptModalContent(script) {
    return `
      <form id="edit-script-form">
        ...
        <div class="form-group">
          <label class="form-label">脚本参数</label>
          <textarea class="form-textarea" id="script-params" placeholder='格式：["param1", "param2", ...]'>${script.params || ''}</textarea>
        </div>
  ...
  async handleAddScript() {
    try {
	  ...
	  let params = document.getElementById('script-params').value.trim();
	  ...
	  // 检查脚本参数是否符合规定
	  if (params) {
		if (!params.startsWith('[') || !params.endsWith(']') || !this.isJSON(params)) {
		  this.showNotification('脚本参数格式不正确', 'warning');
		  return;
	    }
	  } else {
		  params = "[]";
  ...
  getNotificationIcon(type) {
    const icons = {
      success: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#24cb46" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>',
      error: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#f12f0a" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>',
      warning: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#f8e71c" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>',
      info: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#1c6ef8" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>'
    };

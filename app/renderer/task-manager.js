...
class TaskManager {
  ...
  formatSchedule(schedule) {
    switch (schedule.type) {
      case 'interval':
        ...
        if (hours > 0) {
          return `每 ${hours} 小时执行${schedule.once === true ? '【仅一次】' : ''}`;
        } else {
          return `每 ${minutes} 分钟执行${schedule.once === true ? '【仅一次】' : ''}`;
        }
      case 'daily':
        ...
        <div class="form-group" id="interval-config" style="display: none;">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
		    ...
		    <div class="switch-container">
			  <label class="switch">
			    <input type="checkbox" id="switch-check-box">
			    <span class="slider"></span>
			  </label>
			  <span>仅执行一次：<span id="switch-status">关</span></span>
		    </div>			
          </div>
        </div>
  ...
  bindCreateTaskFormEvents() {
    ...
    scheduleTypeSelect?.addEventListener('change', (e) => {
	  ...
	  // 绑定开关按钮
	  if (type === 'interval') {
		const switchCheckbox = document.getElementById('switch-check-box');
		const statusText = document.getElementById('switch-status');
		switchCheckbox.addEventListener('change', (e) => {
		  statusText.textContent = e.target.checked ? '开' : '关';
		});
	  }
    });
    ...
    try {
      ...
      switch (scheduleType) {
        case 'interval':
		  ...
		  const checkboxStatus = document.getElementById('switch-check-box').checked;
          const milliseconds = intervalUnit === 'hours' ? intervalValue * 3600000 : intervalValue * 60000;
          schedule = { type: 'interval', value: milliseconds, once: checkboxStatus };
          break;
      ...
      <form id="edit-task-form">
        ...
        <div class="form-group" id="edit-interval-config" style="display: ${task.schedule.type === 'interval' ? 'block' : 'none'};">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
			...
			<div class="switch-container">
			  <label class="switch">
			    <input type="checkbox" id="switch-check-box" ${task.schedule.once === true ? 'checked' : ''}>
			    <span class="slider"></span>
			  </label>
			  <span>仅执行一次：<span id="switch-status">${task.schedule.once === true ? '开' : '关'}</span></span>
		    </div>	
          </div>
        </div>
